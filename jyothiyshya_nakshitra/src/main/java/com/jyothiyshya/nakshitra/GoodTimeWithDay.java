...
public class GoodTimeWithDay {
public void goodtimeChecker(String day,String work,int hour,int min,int nn) throws JSONException {
...
//						break;
				...
				for(int i=0;i<phalasforday.length();i++) {
					...
					JSONObject endtime1 = (JSONObject) phalasforday.get(i);
					String endtime = phalaforday.getString("endtime");
					JSONObject phalaforday1 = phalaforday.getJSONObject("endtime");
					String endhr = phalaforday1.getString("hr");
					int endhour = Integer.valueOf(endhr);
					if(nn==1) {
					if(hour>endhour && endhour >=0 ) {
				...
				if(work.contains("all")){
					System.out.println("############ Here are all the good phalas ############ ");
